...
text(x = min(datum$x), y = beta0/2, labels = expression(beta[0]), pos = 4, col = "red", font = 2)
text(x = max(datum$x)/2, y = beta0 + beta1 * (max(datum$x)/5), labels = expression(beta[1]), pos = 4, col = "blue", font = 2)

# Add vertical line from origin to regression line for beta0
#abline(v = 0, col = "red", lty = 2)
segments(x0 = 0, y0 = 0, x1 = 0, y1 = beta0, col = "red", lty = 2)
...
segments(x0 = x1, y0 = y1, x1 = x2, y1 = y1, col = "blue", lty = 2) # Run
segments(x0 = x2, y0 = y1, x1 = x2, y1 = y2, col = "blue", lty = 2) # Rise
```
...
y1 <- 20 + 0.5 * x1 + rnorm(n, mean = 0, sd = 8)
...
-	We should be aware of three patterns in regression:
  ...
  - As error goes up, p-value goes up
  -As sample size goes up, p-value goes down
...
```{r help, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r biomass_plot, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r examine-results, echo=TRUE, message=FALSE, warning=FALSE}
