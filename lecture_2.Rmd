...
- We assume there is some hypothesized relationship. We then collect data and perform a statistical test of the null hypothesis: there is no relationship between the X and Y-variables.
...
- <u>Probability that the null hypothesis is true</u>; with the idea being that if the p-value is low, then we might think there is a small probability that the null is true.
...
This is one of the main reasons why Johnson (1999) and many, many other authors don’t like the use of ‘naked p-values’. Too often people use p-values:
...
If the p-value is small (e.g., less the $\alpha$ threshold of 0.05), we conclude there is a statistically significant relationship.
...
- **effect size -- the slope or difference between groups**
...
- We will use the term 'effect size' loosely in this class:
  - Correlation does not mean causation. Just because we observe a relationship between two variables in an observational study does not mean cause and effect.
  - We must do a **manipulative experiment** to demonstrate causation.
...
I dunno. How much does it cost? How do results compare to walking a mile every day, eating healthy, quitting alcohol or tobacco products, etc. (Johnson suggested we should consider more than just statistical probability when evaluating results....)
...
- The problem with power analysis is that we don’t know what the effect is! That’s why we are doing the study... I am not a fan of this.
