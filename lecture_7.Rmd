...
# Create dataframe
biomass <- data.frame(x = x1, y = y1)

# Plot
plot(y ~ x, data = biomass, xlab = "Rainfall (cm)", ylab = "Biomass (kg/ha)")
...
model <- lm(y ~ x, data = biomass)

# Predict values and confidence intervals
predictions <- predict(model, newdata = data.frame(x = sort(x1)), interval = "confidence")
...
lines(sort(x1), predictions[, "lwr"], col = "red", lty = 2)
lines(sort(x1), predictions[, "upr"], col = "red", lty = 2)
```
...
# Predict values and confidence intervals
predictions <- predict(model, newdata = data.frame(x = sort(x1)), interval = "prediction")
...
We will work on how to make predictions and predictions intervals in R during the next class.
