...
However, note that the X-axis title is "Index". This means that the X-values here are in the order of the data, from 1 to *n* -- the sample size. <u>We actually want to make sure that the X-axis in this plot is our actual X-variable</u>, so we can look at how residuals change along the continuous nature of our X-variable.
...
The points (residual error) are following each other! This path is a good indication of autocorrelation. The data are not independent of one another. And it's easier to see here because we have *spread the residuals out* across the graph. 
...
Nah, not really. It never will. But what we are looking for is obvious, <u>**egregious violations**</u> of the assumption of normal error.

For example, let's make this graph for the non-normal data:
...
<u>This is pretty bad! This is a case where we would say "Our data are not normal!"</u> This is what the histogram of the residuals is meant to do: show us non-normal error.

There's is where you might use a log-transformation to fix this -- but it screws up our interpretation, which I don't like.
...
This second graph suggests that the bell curve might be a bit 'skinny' -- which is called **kurtosis**. <u>Kurtosis is when the shape of your distribution is not perfectly normal</u>. It may have *skew* in one direction (a tail), more observations in the middle (bulge), or be 'skinny' and have longer tails on both sides.

This is a rare problem for you to have to deal with, but you can do 'weighted regression' in 'lm()' to account for uneven variance in the model itself (using the 'weights' argument).
...
- For example, note the ACF estimate at a lag = 7 was above the line! But, be careful here. If you randomly have autocorrelation at one lag, think about whether this makes biological sense. Instead, we are looking for<u> **obvious and egregious**</u> evidence of autocorrelation.
