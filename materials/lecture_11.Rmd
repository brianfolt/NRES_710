...
- If the X-variable is categorical, we want to understand the difference between those groups. For example, consider a drug used for medical treatment. We don't want just know whether there is a statistical difference between treatment groups. We want to know **what the effect** of that drug is.
...
A lot of people know that you can extend this model to include multiple X-variables! But what a lot of people don't know, or struggle to recognize, <u>**is *why* you might do this...**</u>
...
- In this day and age, it is easier and more likely for you to publish a paper if you say: "We ran a mixed-effects logistic regression model with interactions and dealt with collinearity and random effects..." Rather than to say: "we ran a t-test". People often equate complicated statistics with good science. Sometimes it does!
- Othertimes, complicated statistics are not necessary. A well-designed experiment does not require complicated statistics!
- I would say, however, that a single, multi-variable analysis is often much more **concise** than iterative versions of univariate analyses. I see a lot of value in making an analysis concise, because it's more simple and easier to understand.

**2. Swamping -- when the effect ($\beta$) of one variable is masked by the effect of another**
...
- If the variable you added explains a lot of noise, then the p-values for other variables should go down a lot as well!
...
- Interactions are kinda... in vogue/popular?! People think it's really cool/interesting to find interactions, and therefore it's useful to test for these
...
<u>**Important point**</u>: when we run this model, it does not change the meanings of the betas at all! They are still interpreted the same ways as we have interpreted them before.
...
So, this 'more complicated model' does not make it harder to interpret these betas -- they still have the same meaning!
...
But the **consequence** of doing this would be that our **error** would be pretty large**.

*Brian overlay a normal curve on top of the entire spread of data.*

Now let's consider what would happen if we run the multi-variable model. What's actually going to happen is we are going to get **two lines!** One for females, and one for males. Let's break up the model and examine this:

**$Y = \beta_0 + \beta_1 Age + \beta_2 * Male + \epsilon \sim N(0, \sigma)$**

**$Y(female) = \beta_0 + \beta_1 Age + \beta_2 * 0 + \epsilon \sim N(0, \sigma)$**
...
**Error:** Our **error** for this line will be **much smaller** than for a line for all the data. 

***Brian indicate reduced error with smaller normal curves overlayed on data and regression line for females.***

The Y-intercept for this line is $\beta_0$, the slope for this line is $\beta_1$
...
So basically what this is saying is that our intercept is now $\beta_0 + \beta_2$. Or, in other words, the **intercept for the male equation** is the **female intercept ($\beta_0$)** plus the **difference of being male ($\beta_2$)**.
...
**Error:** As with the line for females, our **error** for this line will be **much smaller** than for a line for all the data. 

***Brian add normal curve for error in the male line as well.***
...
The meaning of the betas has not changed.

- $\beta_1$ is the slope for both lines
- $\beta_2$ is the difference between the groups.

Notice that $\beta_2$ is not just the difference between the groups at age of zero, <u>it's the difference everywhere</u>! It's constant along the variation of the continuous X-variable. 
...
<u>**Because we have decreased the error related to all of the terms in our model, we have also decreased the p-values for each term**</u>.
...
First, let's look back at our original line fit to all of the data. If we ran an analysis like this without a sex effect, we may have recovered a significant slope for this line, but our **error** would have been much larger, our confidence intervals would have been larger, and our estimate would have been less certain.

For the second scenario, what if we wouldn't have included age in the model?
...
**Q:** Are these groups going to be significantly different from eachother? **Maybe...**

But there is a lot of noise here, and the groups overlap eachother. A linear model of sex alone [e.g., lm(Size ~ Sex)] may not return significance and thus we would fail to reject the null hypothesis of no difference between groups -- when in fact there really is a significant effect of sex! <u>The effect of age that we did not account for might 'swamp out' our ability to measure an effect of sex</u>.
...
So let's load up some data in R and take a look at it. The dataset is located [here](lecture_11_dataset1.csv).
...
Remember, truth for the effect of being male, $\beta_2$, was 2.5. Our results here suggest that this effect is 1.5. But it is statistically significant -- although barely (p = 0.04). Our confidence limits are about ~1.5 (2 * SE). So our best guess from this model is that males are about 1.5 kg larger than females (+/- 1.5). <u>**This is a pretty noisy estimate -- but it does contain truth, which was 2.5.**</u>

Notice the residual standard error is about ~4. Truth was 0.8! So this is about five times larger than truth. There is a lot of extra noise (error) in this simple model! Noise is often not just random variation, but there might be other information that might explains that noise in size. In this case, a lot of the variation in size is driven by age, while very little information is driven by sex. 
...
Let's assume the same dataset as before, but we now have a third group: hermaphrodites. Individual animals with sexual reproductive organs for both sexes. The dataset is located [here](lecture_11_dataset2.csv).
...
Instead, we want to use <u>Type III Sum of Squares</u>, where it always does an F-drop test.

## Testing significance of categorical variables with >2 groups *as a whole*
...
**The point is** we can remove the ambiguity of sequential p-values and force the analysis to use the Type III Sum of Squares and compare the more complicated and more simple models <u>**manually**</u>, as we have shown here.

<u>**Takehome**</u>: if you are performing a multi-variable analysis and want to know the significance of an **entire categorical variable with more than two groups**, use an F-drop test.
