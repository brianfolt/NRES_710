...
We are not ensuring that every field gets every treatment.
...
What if most of the fields that have 'control' plots are also slightly wetter, while other fields with treatments are slightly dryer? This would introduce <u>collinearity</u>! Which makes it difficult to distinguish effects of field from effects of treatments within the field. The **unbalanced design** here introduces collinearity between fixed treatment effects and random field effects. For example, if wet fields have more biomass productivity, then it might make it look like control treatments have more biomass. In this case, field effect becomes a *confounding effect*: it has it's own effect, which is collinear with our treatment.

If we have 30 fields, is there likely to be collinearity? If we increase our sample size to be very large, our collinearity will disappear. Field's will still have their own effects, but it becomes less of a confounding effect because the collinearity decreases across the larger, randomized sample. But with only six fields, it's more likely to get collinearity, because of the small sample size.
...
- However, we might encounter a problem here. Remember that I said that the mixed-effects model were fit using REML. REML involved estimating all the fixed effects first; when it's done, it then estimates the random effects. If we have collinearity, how will it give us unbiased betas for fixed effect of treatment, while statistically controlling for effect of field, *if it's not actually accounting for the effect of field when estimating those betas*.

The point of all this is: while there are cases where you could analyze this design, doing so has limitations and creates problems. In this case, accounting for the field as a random effect does not necessarily solve the problems we have identified.
...
This is what we would call '**pseudoreplication**'! As I mentioned previously, pseudoreplication is:

**Pseudoreplication -- analysis that assumes you have more samples than you really do.**

You might be asking yourself: why aren't these three samples different? They have different measures for biomass for each sample...? But the problem is that the average effect on biomass in these three plots is really high. How do we know whether the biomass is high because of the control effect or because of the field effect? We don't. And that's why we can't treat them as separate samples.

Easy **rule of thumb** for identifying pseudoreplication and samples: your sample is the largest grouping you can create within a given group/treatment. Here, the smallest grouping we can make is the plot, and the largest grouping is the field. Because we can group all of these 'samples' together in each field, then our sample is not the 'plot', but rather it is this largest group -- the field.

**Q:** We have 18 'samples', but I actually just said that we only truly have 6 samples. How can we fix this?
...
- **Q:** Can we include field as a random effect with those data? No, because we only have 1 row per field.
- **This is a perfectly valid way of analyzing these data that eliminates pseudoreplication.** But there is a better way to do it.
...
When we do this, R (and most stats packages) will automatically recognize the pseudoreplication structure in your data, and it will automatically use the field error as the residual error in calculating the effect of treatment. In this case, it's not the between-plot error that is your residual error for calculating the effect of treatment. Rather, it is your between-field error that is your residual error for estimating the effect of treatment, because the field is your sample. 

In general, it is the between-sample error that is your residual error for calculating p-values for any fixed-effect. In this case, the between-sample error is the 'between-field error' -- not the between plot error.
...
The hierarchy should always *spread out* from the top down to the bottom.
...
The terminology doesn't really matter much.
...
But, it is important to understand what <u>pseudoreplication is</u> and <u>how it exists</u>. You cannot get pseudoreplication without nesting. <u>Nesting happens when some random samples only get certain fixed effects</u>. Some fields only get controls, some only get treatment 2, etc. That's the nesting.

Notice that nesting and collinearity are not quite the same.

- For the unbalanced design, this is not nesting, because each field did get more than one treatment.
- Nesting is essentially 'perfect collinearity' -- each field only got a certain type of treatment.

The most simple design -- one plot per field -- is also like ~nesting. But, there's no pseudoreplication and we don't have more than one sample per field, so we can't include a field effect and therefore we don't have to worry about trying to distinguish between the field effect and the treatment effect, because we only have 1 treatment per field.
...
The p-values are much larger. Previously, they were e-12,16, now they are e-4,4. We lost a lot of power. But that's the way it should be! <u>We don't have 18 samples, we only have 6</u>.

The cool thing is that we don't have to tell R that we have pseudoreplication or nesting. We just provide the random effect of field, and R fixes the estimation automatically. 

Make sure you check your degrees of freedom to make sure everything lines up. The degrees of freedom needs to be *less* than the number of samples you have. If it's larger than the number of samples, you have messed something up.
...
**Q:** What kind of design is this? **Randomized block design**. We have plots that are 'blocked', where we randomly assign treatments within; all treatments are assigned within each block.

**Q:** When you run this analyis, what will your block be? Is it a field, or a plot? In truth, we really are most interested in the **plots**. And we could run this model:
...
I don't find how this is written to be super intuitive. *Plots are nested within fields*, but R annotates and this as with 'field | plot' so we can annotate it similarly.
...
```{r fig.height=4, fig.width=5}
# Linear mixed-effects model - correct random-effect structure for this design
results2 <- lme(Biomass ~ Treatment, data = datum, random = ~1|Field/Plot)
summary(results2)

```

Random = tilde 1 | with plots nested within field. We need to tell R that plots are nested within fields.
...
Most importantly, look what happened to our power. The larger the t-value, the more power we have. Before, our t-value for the treatment effect was 27. Now it is 57! We have over doubled our power.
